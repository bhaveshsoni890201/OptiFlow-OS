  ...
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
        <div
          class="w-14 h-14 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-4"
        >
          <svg
        ...
        <h3 class="text-lg font-semibold text-slate-900 mb-2">Session Expiring</h3>
        <p class="text-sm text-slate-500 mb-4">
          Your session will expire in <strong>{{ countdown }}</strong> second{{
            countdown !== 1 ? 's' : ''
          }}
          due to inactivity.
        </p>
