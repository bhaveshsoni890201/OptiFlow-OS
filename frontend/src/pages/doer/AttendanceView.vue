...
const totalHoursToday = computed(() => {
  ...
  if (attendanceStore.checkInTime && attendanceStore.attendanceState === 'checked_in') {
    ...
    if (diff > 0)
      return t('attendance.runningDuration', { hours: Math.floor(diff / 60), minutes: diff % 60 })
  }
  return '--'
})
...
function submitCorrection() {
  correctionErrors.value = {}
  if (!correctionRecord.value)
    correctionErrors.value['record'] = t('attendance.correctionForm.validation.recordRequired')
  if (!correctionReason.value)
    correctionErrors.value['reason'] = t('attendance.correctionForm.validation.reasonRequired')
...
async function fetchAttendance() {
  ...
  startTimeout()
  try {
    await Promise.all([attendanceStore.fetchLogs(), attendanceStore.fetchCorrections()])
      ...
      <div class="space-y-3 text-caption">
        ...
        >
          ...
          <span class="font-semibold text-warning-500">{{
            attendanceStore.pendingCorrectionCount
          }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="xl:col-span-2 space-y-4 sm:space-y-6">
      <div class="bg-white rounded-lg shadow-card p-4 sm:p-6">
        ...
        <div v-else-if="attendanceStore.attendanceState === 'on_leave'" class="text-center py-6">
          ...
          <p class="text-caption text-neutral-500">
            {{ $t('attendance.sickLeaveApproved', { name: 'Anjali Patel' }) }}
          </p>
        </div>
        ...
        <div v-else class="space-y-4">
            ...
            <div class="flex-1 text-center sm:text-left">
              ...
              >
                <span v-if="attendanceStore.checkInTime" class="flex items-center gap-1">
                  ...
                  {{ $t('attendance.inTime', { time: attendanceStore.checkInTime }) }}
                </span>
                <span v-if="attendanceStore.checkOutTime" class="flex items-center gap-1">
                  <ClockIcon class="h-3.5 w-3.5" />
                  {{ $t('attendance.outTime', { time: attendanceStore.checkOutTime }) }}
                </span>
          ...
          >
            ...
            <span class="text-body-strong text-neutral-700">{{
              attendanceStore.workMode === 'wfo'
                ? $t('attendance.workFromOffice')
                : $t('attendance.workFromHome')
            }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="viewMode === 'calendar'" class="bg-white rounded-lg shadow-card p-4 sm:p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center gap-1">
            <button
              class="p-1.5 rounded-lg hover:bg-neutral-100 text-neutral-500"
        ...
        <div class="grid grid-cols-7 gap-1 text-center">
          <div v-for="d in dayKeys" :key="d" class="text-caption text-neutral-400 py-1">
            {{ $t('attendance.days.' + d) }}
          </div>
        ...
        >
          <span class="flex items-center gap-1"
            ><span class="inline-block h-2 w-2 rounded-full bg-success-600" />
            {{ $t('attendance.statusLabels.present') }}</span
          >
          <span class="flex items-center gap-1"
            ><span class="inline-block h-2 w-2 rounded-full bg-warning-500" />
            {{ $t('attendance.statusLabels.late') }}</span
          >
          <span class="flex items-center gap-1"
            ><span class="inline-block h-2 w-2 rounded-full bg-danger-500" />
            {{ $t('attendance.statusLabels.absent') }}</span
          >
          <span class="flex items-center gap-1"
            ><span class="inline-block h-2 w-2 rounded-full bg-info-600" />
            {{ $t('attendance.statusLabels.leave') }}</span
          >
        </div>
              ...
              <div>
                ...
                <p class="text-caption text-neutral-500">
                  ...
                  <span v-if="log.work_mode" class="ml-1"
                    >·
                    {{
                      log.work_mode === 'wfo' ? $t('attendance.office') : $t('attendance.home')
                    }}</span
                  >
                </p>
              </div>
      ...
      <div v-if="viewMode === 'list'" class="bg-white rounded-lg shadow-card p-4 sm:p-6">
        ...
        <OptEmptyState
          ...
          :description="$t('attendance.noLogs')"
        />
      ...
      >
        ...
        <div v-if="correctionSubmitted" class="p-6 text-center">
          ...
          <p class="text-body-strong text-neutral-900 mb-1">
            {{ $t('attendance.correctionForm.submitted') }}
          </p>
          <p class="text-caption text-neutral-500">
        ...
        <form v-else class="p-4 space-y-4" @submit.prevent="submitCorrection">
          <div>
            <label class="block text-caption font-semibold text-neutral-700 mb-1">{{
              $t('attendance.correctionForm.selectRecord')
            }}</label>
            <select
              :id="'input-record'"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-caption font-semibold text-neutral-700 mb-1">{{
                $t('attendance.correctionForm.correctedCheckIn')
              }}</label>
              <input
            ...
            <div>
              <label class="block text-caption font-semibold text-neutral-700 mb-1">{{
                $t('attendance.correctionForm.correctedCheckOut')
              }}</label>
              <input
          ...
          <div>
            <label class="block text-caption font-semibold text-neutral-700 mb-1"
              >{{ $t('attendance.correctionForm.reason') }}
          ...
          <div>
            <label class="block text-caption font-semibold text-neutral-700 mb-1">{{
              $t('attendance.correctionForm.attachment')
            }}</label>
            <label
            ...
            >
              ...
              <span>{{
                correctionAttachment
                  ...
                  : $t('attendance.correctionForm.uploadDocument')
              }}</span>
              <input
                type="file"
