...
function formatDueDate(dueDate: string) {
  const d = new Date(dueDate)
  const now = new Date()
  if (d.toDateString() === now.toDateString()) return `Today ${formatTime(d)}`
...
async function fetchMyTasks() {
  ...
  try {
    ...
    delegations.value = [...taskStore.delegationTasks.filter((d) => d.assigned_to === empId.value)]
...
<template>
  ...
  <div v-if="activeFiltersCount > 0" class="flex flex-wrap gap-2 mb-3">
    ...
    <OptChip v-if="selectedPriority" variant="default" removable @remove="removeFilter('priority')">
      {{ selectedPriority }}
    </OptChip>
  ...
  <div v-else class="space-y-2">
    <div
      ...
      :key="task.id"
      :class="[
        'card p-4 hover:shadow-card-hover transition-all active:scale-[0.99] relative overflow-hidden',
        completing.has((task as ChecklistItem).id) ? 'animate-fade-out-down' : '',
      ]"
