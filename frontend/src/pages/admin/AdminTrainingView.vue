...
async function loadTrainingData() {
  ...
  try {
    ...
    assignments.value = data.map((a) => ({
      id: a.id,
      employee:
        adminStore.employees.find((e) => e.employee_id === a.employee_id)?.name || a.employee_id,
    ...
    const grouped = new Map<
      string,
      { assigned: number; completed: number; deadline: string; type: Assignment['status'] }
    >()
    for (const a of assignments.value) {
      const g = grouped.get(a.title) || {
        assigned: 0,
        completed: 0,
        deadline: a.deadline,
    ...
    <template v-else>
      ...
      <div v-if="activeTab === 'oversight'" class="space-y-4">
        <OptEmptyState
          ...
          type="training"
          title="All caught up"
          description="No overdue training assignments"
        />
