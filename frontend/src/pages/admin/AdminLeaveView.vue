...
const balances = computed(() => {
  ...
  return adminStore.employees
    ...
    .map((e) => {
      const empLeaves = empRequests.filter(
        (r) => r.employee_id === e.employee_id && r.status === 'approved',
      )
...
const activeTransfers = computed(() => workflowStore.buddyTransfers.filter((t) => !t.reverted))
const pendingReverts = computed(() => workflowStore.buddyTransfers.filter((t) => t.reverted))
...
async function loadLeaveData() {
  ...
  error.value = null
  try {
    await Promise.all([workflowStore.fetchLeaveRequests(), adminStore.fetchEmployees()])
    ...
    <template v-else>
      <div class="flex items-center justify-between">
        ...
        <span
          ...
          class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-orange-50 text-orange-700 border border-orange-200"
        >
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-5">
        ...
        <div class="grid grid-cols-7 gap-1.5">
          ...
          >
            ...
            <span v-if="d.count > 0" class="block text-xs mt-0.5 opacity-70"
              >{{ d.count }} away</span
            >
          </div>
        </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          ...
          <OptFilterBar v-model="filterModel" :filters="filterConfig" compact />
        </div>
        <OptTable
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <OptEmptyState
            ...
            description="There are no buddy transfers in progress."
          />
          <div
          ...
          >
            <div>
              <p class="text-sm font-medium text-slate-800">
                {{ bt.original_owner }} → {{ bt.buddy_owner }}
              </p>
              <p class="text-xs text-slate-500">{{ bt.transfer_start }} — {{ bt.transfer_end }}</p>
            </div>
            <span
          ...
          <OptEmptyState
            v-if="pendingReverts.length === 0"
            type="leave"
            title="No pending reverts"
            description="No reverts are awaiting review."
          />
          <div
