...
const captains = computed(() =>
  (adminStore.employees ?? []).filter(
    (e) =>
      (e.roles.includes('captain') || e.roles.includes('admin')) &&
      e.employee_id !== memberId.value,
  ),
)
...
async function confirmTransfer() {
  ...
  const newCaptain = adminStore.employees.find(
    (e) => e.employee_id === transferModal.value!.newCaptainId,
  )
  rootStore.addToast({
    type: 'success',
  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <div v-else-if="error" class="flex items-center justify-center min-h-[60vh]">
      <div class="text-center">
        ...
        <button class="mt-3 text-sm text-blue-600 hover:underline" @click="loadMember()">
          Retry
        </button>
      </div>
    </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-5">
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4">
          ...
          <div class="flex-1 text-center sm:text-left">
            ...
            <div
              class="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-2 text-xs text-slate-400"
            >
              ...
              <span class="inline-flex items-center gap-1"> ID: {{ member.employee_id }} </span>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            >
              <span
                ...
                :class="member.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'"
              />
