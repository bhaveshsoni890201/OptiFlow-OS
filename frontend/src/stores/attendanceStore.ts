...
function persistState(data: {
  ...
  try {
    ...
    sessionStorage.setItem(
      'optiflow-attendance',
      JSON.stringify({ ...existing, ...data, _date: new Date().toISOString().split('T')[0] }),
    )
  } catch {
...
function loadPersistedState(): {
  ...
  try {
    ...
    if (data._date !== new Date().toISOString().split('T')[0]) {
      try {
        sessionStorage.removeItem('optiflow-attendance')
      } catch {
        /* ignore */
      }
      return null
...
export const useAttendanceStore = defineStore('attendance', () => {
  ...
  function getCorrectionStatus(recordId: string): 'pending' | 'approved' | 'rejected' | undefined {
