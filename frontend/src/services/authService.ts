...
class AuthService extends BaseService {
  ...
  async login(employeeId: string, password: string): Promise<LoginResponse> {
    try {
      if (isMockMode()) {
        const { employees } = await import('../mock/employees').catch(() => ({
          employees: [] as Employee[],
        }))
        const normalize = (id: string) => id.replace(/-/g, '')
        const emp = employees.find(
          (e: Employee) => normalize(e.employee_id) === normalize(employeeId),
        )
      ...
      return await this.mutate<LoginResponse>('post', endpoints.auth.login, {
  ...
  async verifyOtp(mobile: string, otp: string): Promise<LoginResponse> {
    try {
      ...
      throw err instanceof ServiceError
        ? err
        : new ServiceError('OTP verification failed', 'AUTH_OTP_ERROR')
    }
  }
  ...
  async getProfile(): Promise<LoginResponse | null> {
    try {
      ...
      throw err instanceof ServiceError
        ? err
        : new ServiceError('Failed to fetch profile', 'AUTH_PROFILE_ERROR')
    }
  }
  ...
  async forgotPassword(employeeId: string): Promise<void> {
    try {
      ...
      throw err instanceof ServiceError
        ? err
        : new ServiceError('Forgot password request failed', 'AUTH_FORGOT_ERROR')
    }
  }
  ...
  async resetPassword(token: string, password: string): Promise<void> {
    try {
      ...
      throw err instanceof ServiceError
        ? err
        : new ServiceError('Password reset failed', 'AUTH_RESET_ERROR')
    }
  }
}
...
export const login = (employeeId: string, password: string) =>
  authService.login(employeeId, password)
